...
- (void)setMessageFrame:(UUMessageFrame *)messageFrame
{
    ...
    FDSJChatModel *chatModel = messageFrame.chatModel;
    
    // 1、设置时间
    self.dateLabel.text = chatModel.created_at;
    ...
    [self.headImageButton setBackgroundImageForState:UIControlStateNormal
                                          withURL:[NSURL URLWithString:chatModel.avatar]
                                 placeholderImage:[UIImage uu_imageWithName:@"headImage.jpeg"]];
    
    // 3、设置下标
    self.namelabel.text = chatModel.nick;
...
//    }
    ...
    [self.btnContent setTitle:chatModel.content forState:UIControlStateNormal];
